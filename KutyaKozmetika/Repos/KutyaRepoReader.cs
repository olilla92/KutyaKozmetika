...
namespace KutyaKozmetika.Repos
{
    public class KutyaRepoReader
    {
        ...
        public bool ReadFileFrom(string filename)
        {
            try
            {
                foreach (var rec in File.ReadAllLines(filename).Skip(1))
                {
                    _kutyak.Add(new KutyaRecord(rec));
                }
            }
            catch (Exception ex)
            {
